    ...
    <AppShell>
        <template #header>
            <div class="flex items-center gap-3">
                <h1 class="text-base font-semibold text-gray-800 dark:text-slate-200">Capitation</h1>
                <span class="inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300">
                    {{ currentMonthYear }}
                </span>
            </div>
        </template>
        ...
        <div class="px-6 py-5 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4 flex items-start gap-3">
                    ...
                    <div class="min-w-0">
                        <p class="text-xs text-gray-500 dark:text-slate-400 truncate">Current Month Total</p>
                ...
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4 flex items-start gap-3">
                    <div class="shrink-0 p-2 rounded-lg bg-indigo-50 dark:bg-indigo-900/30">
                        <UsersIcon class="w-5 h-5 text-indigo-600 dark:text-indigo-400" aria-hidden="true" />
                    </div>
                    <div class="min-w-0">
                        <p class="text-xs text-gray-500 dark:text-slate-400 truncate">Participant Count</p>
                        <p class="text-xl font-bold text-gray-800 dark:text-slate-100 mt-0.5">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4 flex items-start gap-3">
                    <div class="shrink-0 p-2 rounded-lg bg-emerald-50 dark:bg-emerald-900/30">
                        <ChartBarIcon class="w-5 h-5 text-emerald-600 dark:text-emerald-400" aria-hidden="true" />
                    </div>
                    <div class="min-w-0">
                        <p class="text-xs text-gray-500 dark:text-slate-400 truncate">Avg RAF Score</p>
                        <p class="text-xl font-bold text-gray-800 dark:text-slate-100 mt-0.5">
                            {{ kpis.avg_raf_score != null ? Number(kpis.avg_raf_score).toFixed(3) : '-' }}
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    <table class="min-w-full text-sm">
                        <thead class="bg-gray-50 dark:bg-slate-700/50">
                            <tr>
                                ...
                                <th class="px-4 py-2.5 text-right text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide">HCC Risk</th>
                                <th class="px-4 py-2.5 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide">Payment Status</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                            ...
                            >
                                <td class="px-4 py-3">
                                    ...
                                    >
                                        <span class="font-medium text-blue-600 dark:text-blue-400 group-hover:underline">
                                            {{ participantName(rec) }}
                                        </span>
                                        <span class="ml-1.5 text-xs text-gray-400 dark:text-slate-500">
                                            {{ rec.participant.mrn }}
                                        </span>
                                    </button>
                                ...
                                <td class="px-4 py-3 text-right text-gray-600 dark:text-slate-400 tabular-nums">
                                    {{ rec.hcc_risk_score != null ? Number(rec.hcc_risk_score).toFixed(3) : '-' }}
                                </td>
                                <td class="px-4 py-3">
                                    <span
                                        v-if="rec.payment_status"
                                        :class="['inline-flex px-2 py-0.5 rounded text-xs font-medium capitalize', paymentStatusClass(rec.payment_status)]"
                                    >
                                        {{ rec.payment_status }}
                                    </span>
                                    <span v-else class="text-gray-400 dark:text-slate-500">-</span>
