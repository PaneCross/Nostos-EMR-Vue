...
const openEditBaa = (r: BaaRecord) => {
    ...
    baaForm.value = {
        ...
        baa_signed_date: r.baa_signed_date ?? '',
        baa_expiration_date: r.baa_expiration_date ?? '',
        status: r.status,
    ...
    <AppShell>
        ...
        <div class="max-w-5xl mx-auto px-6 py-8">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-slate-400">
                            BAA/SRA tracking and encryption status
                        </p>
                    </div>
                </div>
                <div class="flex flex-wrap gap-2 justify-end">
                    <span
                        v-if="props.posture.expired_baa_count > 0"
                    ...
                    <span
                        v-if="props.posture.expiring_soon_count > 0"
                    ...
                    <span
                        :class="
                            props.posture.field_encryption
                                ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300'
                    ...
                    >
                        <CheckCircleIcon
                            v-if="props.posture.field_encryption"
                            class="w-3.5 h-3.5"
                        />
                        <ExclamationTriangleIcon v-else class="w-3.5 h-3.5" />
            ...
            <div class="border-b border-gray-200 dark:border-slate-700 mb-6">
                <nav class="flex gap-6">
                    <button
                        v-for="tab in ['baa', 'sra', 'encryption'] as const"
                        :key="tab"
                        :class="
                            activeTab === tab
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                            ...
                            >
                                ...
                                <td class="px-4 py-3 text-gray-600 dark:text-slate-400 capitalize">
                                    {{ r.vendor_type.replace('_', ' ') }}
                                </td>
                                <td class="px-4 py-3">
                                    <span
                                        :class="baaStatusBg(r.status)"
                                ...
                                <td class="px-4 py-3 text-gray-600 dark:text-slate-400">
                                    {{ formatDate(r.baa_expiration_date) }}
                                </td>
                                <td class="px-4 py-3">
                                    <button
                                        ...
                                        :aria-label="`Edit BAA for ${r.vendor_name}`"
                                        @click="openEditBaa(r)"
                                    >
                                        Edit
                                    </button>
            ...
            <div v-if="activeTab === 'sra'">
                ...
                <div class="space-y-4">
                    ...
                    >
                        <div class="flex items-start justify-between">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <button
                                    class="text-xs px-2 py-1 rounded border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors"
                                    :aria-label="`Edit SRA from ${r.sra_date}`"
                                    @click="openEditSra(r)"
                                >
                                    Edit
                                </button>
                        ...
                        <div
                            v-if="r.findings_summary"
                            class="mt-2 text-sm text-gray-600 dark:text-slate-400 border-t border-gray-100 dark:border-slate-700 pt-2"
                        >
                            {{ r.findings_summary }}
                        </div>
                    ...
                    <div
                        v-if="props.sraRecords.length === 0"
                        class="py-12 text-center text-gray-500 dark:text-slate-400"
                    >
                        No SRA records.
                    </div>
            ...
            <div v-if="activeTab === 'encryption'">
                ...
                >
                    ...
                    >
                        <h3 class="font-semibold text-gray-700 dark:text-slate-300 text-sm">
                            Runtime Encryption Checks (HIPAA 45 CFR 164.312)
                        </h3>
                    </div>
                    <div class="divide-y divide-gray-100 dark:divide-slate-700">
                        <div
                            v-for="(check, key) in props.encryptionStatus.checks"
                            :key="key"
                ...
                >
                    ...
                    <ul
                        class="mt-2 text-sm text-blue-700 dark:text-blue-400 space-y-1 list-disc list-inside"
                    >
                        <li>
                            Set <code class="font-mono">SESSION_ENCRYPT=true</code> in production
                            .env
                        </li>
                        <li>
                            Set <code class="font-mono">DB_SSLMODE=require</code> for encrypted DB
                            connections
                        </li>
                        <li>
                            Configure <code class="font-mono">FILESYSTEM_DISK=s3</code> with SSE-KMS
                            for documents
                        </li>
                        <li>
                            Set <code class="font-mono">REDIS_PASSWORD</code> to secure Redis
                            connections
                        </li>
                    </ul>
                </div>
        ...
        >
            ...
            >
                ...
                <form class="space-y-3" @submit.prevent="submitBaa">
                    <div>
                        <label
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            >
                                ...
                                <option v-for="t in props.vendorTypes" :key="t" :value="t">
                                    {{ t.replace('_', ' ') }}
                                </option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select
                                id="baa-status"
                                v-model="baaForm.status"
                                class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm text-gray-700 dark:text-slate-300"
                            >
                                <option v-for="s in props.baaStatuses" :key="s" :value="s">
                                    {{ s.replace('_', ' ') }}
                                </option>
                            </select>
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input
                                id="baa-signed"
                                v-model="baaForm.baa_signed_date"
                                type="date"
                                class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm"
                            />
                        </div>
                        <div>
                            ...
                            <input
                                id="baa-expires"
                                v-model="baaForm.baa_expiration_date"
                                type="date"
                                class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm"
                            />
                        </div>
                    ...
                    <div class="flex items-center gap-2">
                        <input
                            id="baa-phi"
                            v-model="baaForm.phi_accessed"
                            type="checkbox"
                            class="rounded border-gray-300 dark:border-slate-600"
                        />
                        <label for="baa-phi" class="text-sm text-gray-700 dark:text-slate-300"
                            >Vendor accesses PHI</label
                        >
                    </div>
                    <div>
                        <label
                    ...
                    <div class="flex justify-end gap-3 pt-2">
                        <button
                            ...
                            @click="showBaaModal = false"
                        >
                            Cancel
                        </button>
                        <button
                            type="submit"
                            :disabled="baaSubmitting"
        ...
        >
            ...
            >
                ...
                <form class="space-y-3" @submit.prevent="submitSra">
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1"
                    ...
                    <div>
                        ...
                        <textarea
                            id="sra-scope"
                            v-model="sraForm.scope_description"
                            required
                            rows="2"
                            class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm"
                        ></textarea>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select
                                id="sra-risk"
                                v-model="sraForm.risk_level"
                                class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm text-gray-700 dark:text-slate-300"
                            >
                                <option v-for="l in props.sraRiskLevels" :key="l" :value="l">
                                    {{ l }}
                                </option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select
                                id="sra-status"
                                v-model="sraForm.status"
                                class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm text-gray-700 dark:text-slate-300"
                            >
                                <option v-for="s in props.sraStatuses" :key="s" :value="s">
                                    {{ s.replace('_', ' ') }}
                                </option>
                            </select>
                        </div>
                    ...
                    <div>
                        ...
                        <textarea
                            id="sra-findings"
                            v-model="sraForm.findings_summary"
                            rows="3"
                            class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm"
                        ></textarea>
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-2">
                        <button
                            type="button"
                            class="px-4 py-2 text-sm rounded-lg text-gray-600 dark:text-slate-400 hover:text-gray-900 dark:hover:text-slate-200 transition-colors"
                            @click="showSraModal = false"
                        >
                            Cancel
                        </button>
                        <button
                            type="submit"
                            :disabled="sraSubmitting"
                            class="px-4 py-2 text-sm rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-medium disabled:opacity-50 transition-colors"
                        >
