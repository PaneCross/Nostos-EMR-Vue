...
interface Address {
  id: number; address_type: string; street_line_1: string; street_line_2: string | null
  city: string; state: string; zip_code: string; is_primary: boolean
}
interface Contact {
...
interface Insurance {
  id: number; payer_type: string; member_id: string | null; plan_name: string | null
  effective_date: string | null; term_date: string | null; is_active: boolean
}
interface AuditEntry { id: number; action: string; description: string | null; user_id: number | null; created_at: string }
interface Problem {
  id: number; icd10_code: string; icd10_description: string; category: string | null
  status: string; onset_date: string | null; is_primary_diagnosis: boolean
}
interface Allergy {
  id: number; allergen_name: string; allergy_type: string; severity: string
  reaction_description: string | null; is_active: boolean
}
interface Vital {
  id: number; recorded_at: string; systolic_bp: number | null; diastolic_bp: number | null
  heart_rate: number | null; respiratory_rate: number | null; temperature_f: number | null
  weight_lbs: number | null; oxygen_saturation: number | null; pain_scale: number | null
  blood_glucose: number | null
}

type Tab = 'overview'|'chart'|'vitals'|'assessments'|'problems'|'allergies'|'adl'|'careplan'
  |'medications'|'emar'|'med-recon'|'immunizations'|'procedures'|'sdoh'|'wounds'|'lab-results'
  |'orders'|'contacts'|'flags'|'insurance'|'documents'|'audit'|'transfers'
...
const auth = computed(() => (page.props as Record<string, unknown>).auth as {
...
onMounted(() => { activeTab.value = getInitialTab() })
...
async function requestBreakGlass() {
  ...
  bgError.value = null
  router.post(`/participants/${props.participant.id}/break-glass`, {}, {
    preserveScroll: true,
    onError: (e: Record<string, string>) => { bgError.value = e.message ?? 'Failed.' },
    onFinish: () => { bgLoading.value = false },
  })
}
...
<template>
  ...
  <AppShell>
    <div class="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <!-- Participant header -->
      <div class="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-5 mb-4 flex items-start gap-5">
        <!-- Photo / initials -->
        <div class="w-16 h-16 rounded-full bg-gray-200 dark:bg-slate-700 flex-shrink-0 overflow-hidden flex items-center justify-center">
          <img v-if="participant.photo_url" :src="participant.photo_url" :alt="participant.first_name" class="w-full h-full object-cover" />
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-3 flex-wrap">
            ...
            <span :class="`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLORS[participant.status] ?? 'bg-gray-100 text-gray-600'}`">
              {{ participant.status }}
            </span>
            <span v-if="participant.has_dnr" class="text-xs px-2 py-0.5 rounded-full font-bold bg-red-100 dark:bg-red-900/60 text-red-700 dark:text-red-300 border border-red-300">DNR</span>
            <span
            ...
            >
              ...
              {{ lifeThreateningAllergyCount === 1 ? 'Life-Threatening Allergy' : 'Life-Threatening Allergies' }}
            </span>
          </div>
          <div class="mt-1.5 flex flex-wrap gap-x-4 gap-y-1 text-sm text-gray-500 dark:text-slate-400">
            <span>MRN: <span class="font-mono text-gray-800 dark:text-slate-200">{{ participant.mrn }}</span></span>
            <span v-if="participant.dob">DOB: {{ fmtDate(participant.dob) }} ({{ calcAge(participant.dob) }} y/o)</span>
            <span v-if="participant.gender">{{ participant.gender }}</span>
            <span v-if="participant.site_name">Site: {{ participant.site_name }}</span>
            <span v-if="participant.enrollment_date">Enrolled: {{ fmtDate(participant.enrollment_date) }}</span>
          </div>
        </div>
      ...
      <div class="border-b border-gray-200 dark:border-slate-700">
        <div class="flex items-center gap-0.5 overflow-x-auto">
          <span class="text-xs font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wider pr-2 flex-shrink-0">Clinical</span>
          <button
      ...
      <div class="mb-4 border-b border-gray-200 dark:border-slate-700">
        <div class="flex items-center gap-0.5 overflow-x-auto">
          <span class="text-xs font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wider pr-2 flex-shrink-0">Admin</span>
          <button
      ...
      <div class="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-5">
        <KeepAlive>
          <ChartTab v-if="activeTab === 'chart'" :participant-id="participant.id" :note-templates="noteTemplates" :has-multiple-sites="hasMultipleSites" />
          <VitalsTab v-else-if="activeTab === 'vitals'" :participant-id="participant.id" :initial-vitals="vitals" :completed-transfers="completedTransfers" />
          <AssessmentsTab v-else-if="activeTab === 'assessments'" :participant-id="participant.id" />
          <MedicationsTab v-else-if="activeTab === 'medications'" :participant-id="participant.id" />
          <EmarTab v-else-if="activeTab === 'emar'" :participant-id="participant.id" />
          <MedReconTab v-else-if="activeTab === 'med-recon'" :participant-id="participant.id" />
          <ProblemsTab v-else-if="activeTab === 'problems'" :participant-id="participant.id" :initial-problems="problems" :icd10-codes="icd10Codes" />
          <AllergiesTab v-else-if="activeTab === 'allergies'" :participant-id="participant.id" :initial-allergies="allergies" />
          <AdlTab v-else-if="activeTab === 'adl'" :participant-id="participant.id" />
          <CarePlanTab v-else-if="activeTab === 'careplan'" :participant-id="participant.id" />
          <OrdersTab v-else-if="activeTab === 'orders'" :participant-id="participant.id" :user-dept="dept" :is-super-admin="isSuperAdmin" />
          <ImmunizationsTab v-else-if="activeTab === 'immunizations'" :participant-id="participant.id" />
          <ProceduresTab v-else-if="activeTab === 'procedures'" :participant-id="participant.id" />
          <WoundsTab v-else-if="activeTab === 'wounds'" :participant-id="participant.id" />
          <LabResultsTab v-else-if="activeTab === 'lab-results'" :participant-id="participant.id" :user-dept="dept" :user-is-super-admin="isSuperAdmin" />
          <OverviewTab v-else-if="activeTab === 'overview'" :participant="participant" :addresses="addresses" :contacts="contacts" :flags="flags" />
          <ContactsTab v-else-if="activeTab === 'contacts'" :participant-id="participant.id" :initial-contacts="contacts" />
          <FlagsTab v-else-if="activeTab === 'flags'" :participant-id="participant.id" :initial-flags="flags" />
          <InsuranceTab v-else-if="activeTab === 'insurance'" :insurances="insurances" />
          <DocumentsTab v-else-if="activeTab === 'documents'" :participant-id="participant.id" />
          <SdohTab v-else-if="activeTab === 'sdoh'" :participant-id="participant.id" />
          <TransfersTab v-else-if="activeTab === 'transfers'" :participant-id="participant.id" />
          <GrievancesTab v-else-if="activeTab === 'grievances'" :participant-id="participant.id" />
          <ConsentsTab v-else-if="activeTab === 'consents'" :participant-id="participant.id" />
          <DisenrollmentTab v-else-if="activeTab === 'disenrollment'" :participant-id="participant.id" />
