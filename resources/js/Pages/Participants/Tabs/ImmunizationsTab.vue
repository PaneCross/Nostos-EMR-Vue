...
const immunizations = ref<Immunization[]>(props.immunizations)
...
async function submit() {
    if (!form.value.vaccine_name.trim()) {
        error.value = 'Vaccine name is required.'
        return
    }
    saving.value = true
    error.value = ''
        ...
        >
            ...
            <div class="mb-3">
                ...
                <datalist id="common-vaccines">
                    <option v-for="v in COMMON_VACCINES" :key="v" :value="v"></option>
                </datalist>
            </div>
            <div class="grid grid-cols-2 gap-3 mb-3">
                <div>
                    <label class="block text-xs font-medium text-gray-700 dark:text-slate-300 mb-1"
                        >Administered Date</label
                    >
                    <input
            ...
            <div v-if="form.vis_given" class="mb-3">
                <label class="block text-xs font-medium text-gray-700 dark:text-slate-300 mb-1"
                    >VIS Publication Date</label
                >
                <input
                    v-model="form.vis_publication_date"
                    type="date"
                    class="w-full text-sm border border-gray-300 dark:border-slate-600 rounded-md px-2 py-1.5 bg-white dark:bg-slate-700"
                />
            </div>
            ...
            >
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2">
                        ...
                        <span
                            v-if="imm.vis_given"
                            class="inline-flex items-center gap-0.5 text-xs text-green-600 dark:text-green-400"
                        >
                            <CheckCircleIcon class="w-3 h-3" /> VIS
                        </span>
                    </div>
                    <div
                        class="text-xs text-gray-400 dark:text-slate-500 mt-0.5 flex gap-2 flex-wrap"
                    >
                        ...
                        <span v-if="imm.administered_by"
                            >by {{ imm.administered_by.first_name }}
                            {{ imm.administered_by.last_name }}</span
                        >
                    </div>
                    <p v-if="imm.notes" class="text-xs text-gray-500 dark:text-slate-500 mt-0.5">
                        {{ imm.notes }}
                    </p>
                </div>
            </div>
