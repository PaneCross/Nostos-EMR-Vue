            ...
            >
                ...
                >
                    <ExclamationTriangleIcon
                        v-if="result.is_critical"
                        class="w-4 h-4 text-red-500 dark:text-red-400 shrink-0"
                    />
                    <span class="text-sm font-medium text-gray-900 dark:text-slate-100 flex-1">{{
                        result.panel_name
                    }}</span>
                    <span
                    ...
                    <span class="text-xs text-gray-400 dark:text-slate-500">{{
                        fmtDate(result.resulted_date)
                    }}</span>
                    <span
                        v-if="result.lab_name"
                        class="text-xs text-gray-400 dark:text-slate-500"
                        >{{ result.lab_name }}</span
                    >
                    <ChevronDownIcon
                        :class="[
                            'w-4 h-4 text-gray-400 transition-transform',
                            expandedIds.has(result.id) ? 'rotate-180' : '',
                        ]"
                    />
                </button>
                ...
                <!-- Expanded detail -->
                <div
                    v-if="expandedIds.has(result.id)"
                    class="border-t border-gray-100 dark:border-slate-700 px-4 py-3"
                >
                    <div
                        class="text-xs text-gray-400 dark:text-slate-500 mb-3 flex gap-4 flex-wrap"
                    >
                        <span v-if="result.ordering_provider"
                            >Provider: {{ result.ordering_provider }}</span
                        >
                        <span v-if="result.ordered_date"
                            >Ordered: {{ fmtDate(result.ordered_date) }}</span
                        >
                    </div>
                        ...
                        <table class="w-full text-xs border-collapse">
                            <thead>
                                <tr class="border-b border-gray-100 dark:border-slate-700">
                                    <th
                                        class="text-left py-1 font-medium text-gray-500 dark:text-slate-400"
                                    >
                                        Component
                                    </th>
                                    <th
                                        class="text-center py-1 font-medium text-gray-500 dark:text-slate-400"
                                    >
                                        Value
                                    </th>
                                    <th
                            ...
                            <tbody>
                                <tr
                                    ...
                                    :key="comp.id"
                                    class="border-b border-gray-50 dark:border-slate-700/50"
                                >
                                    <td class="py-1 text-gray-700 dark:text-slate-300">
                                        {{ comp.component_name }}
                                    </td>
                                    <td :class="['text-center py-1', flagColor(comp.flag)]">
                                        {{ comp.value ?? '-'
                                        }}<span v-if="comp.unit" class="text-gray-400 ml-0.5">{{
                                            comp.unit
                                        }}</span>
                                    </td>
                                    <td class="text-center py-1 text-gray-400 dark:text-slate-500">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        >
                            ...
                            <span v-if="result.reviewed_by">
                                by {{ result.reviewed_by.first_name }}
                                {{ result.reviewed_by.last_name }}</span
                            >
                        </span>
                    </div>
                </div>
