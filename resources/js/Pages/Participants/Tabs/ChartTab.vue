...
async function submitNote() {
    ...
    router.post(
        `/participants/${props.participantId}/notes`,
        {
...
async function signNote(noteId: number) {
    router.patch(
        `/participants/${props.participantId}/notes/${noteId}/sign`,
        {},
        {
            preserveScroll: true,
            onSuccess: loadNotes,
...
<template>
    <div>
        ...
        <div class="flex flex-wrap gap-2 mb-4">
            <select
                v-model="filterDept"
                class="text-xs border border-gray-200 dark:border-slate-600 rounded-lg px-2 py-1.5 bg-white dark:bg-slate-800 text-gray-700 dark:text-slate-300"
                @change="loadNotes"
            >
            ...
            >
                ...
                >
                    <div class="flex items-center gap-3 flex-wrap">
                        ...
                        <span
                            :class="`text-xs px-1.5 py-0.5 rounded-full font-medium ${STATUS_CLASSES[note.status] ?? 'bg-gray-100 text-gray-600'}`"
                            >{{ note.status }}</span
                        >
                        <span
                            v-if="hasMultipleSites && note.site_name"
                            class="text-xs px-1.5 py-0.5 rounded bg-indigo-100 dark:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300"
                            >{{ note.site_name }}</span
                        >
                    </div>
                ...
                <div v-if="expanded.has(note.id)" class="px-4 py-3 bg-white dark:bg-slate-800">
                    <p class="text-xs text-gray-500 dark:text-slate-400 mb-2">
                        ...
                        <span v-if="note.cosigner_name">
                            | Co-signed: {{ note.cosigner_name }}</span
                        >
                        <span v-if="note.signed_at">
                            | Signed: {{ fmtDatetime(note.signed_at) }}</span
                        >
                    </p>
                    ...
                    <div v-if="note.addendums.length > 0" class="mt-3 space-y-2">
                        <p
                            class="text-xs font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wide"
                        >
                            Addendums
                        </p>
                        <div
                            v-for="add in note.addendums"
                            :key="add.id"
                            class="border-l-2 border-blue-300 dark:border-blue-700 pl-3"
                        >
                            <p class="text-xs text-gray-500 dark:text-slate-400">
                                {{ add.author_name ?? 'Unknown' }} -
                                {{ fmtDatetime(add.created_at) }}
                            </p>
                            <p
                                class="text-sm text-gray-800 dark:text-slate-200 whitespace-pre-wrap"
                            >
                                {{ add.content }}
                            </p>
                        </div>
        ...
        >
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-lg p-6">
                ...
                <p v-if="modalError" class="text-sm text-red-600 dark:text-red-400 mb-3">
                    {{ modalError }}
                </p>
                <div class="space-y-4">
                    <div>
