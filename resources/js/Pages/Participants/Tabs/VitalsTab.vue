...
function resetForm() {
    Object.keys(form.value).forEach((k) => {
        ;(form.value as Record<string, string>)[k] = ''
    })
...
<template>
    <div>
        ...
        <div
            v-if="showForm"
            class="mb-4 bg-gray-50 dark:bg-slate-700/50 border border-gray-200 dark:border-slate-600 rounded-lg p-4"
        >
            <h4 class="text-xs font-semibold text-gray-700 dark:text-slate-300 mb-3">
                New Vitals Entry
            </h4>
            <p v-if="formError" class="text-sm text-red-600 dark:text-red-400 mb-2">
                {{ formError }}
            </p>
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <div>
                    ...
                    <input
                        ...
                        min="50"
                        max="280"
                        class="w-full text-sm border border-gray-300 dark:border-slate-600 rounded-lg px-3 py-1.5 bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100"
                        placeholder="mmHg"
                    />
                </div>
                <div>
            ...
            <div class="flex justify-end gap-2 mt-3">
                <button
                    class="text-xs px-3 py-1.5 bg-gray-200 dark:bg-slate-600 text-gray-700 dark:text-slate-200 rounded-lg hover:bg-gray-300 transition-colors"
        ...
        <div v-else class="border border-gray-200 dark:border-slate-700 rounded-lg overflow-hidden">
            <table class="text-sm w-full overflow-x-auto">
                <thead class="bg-gray-50 dark:bg-slate-700/50">
                    <tr>
                        <th
                            class="px-3 py-2 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide whitespace-nowrap"
                        >
                            Date/Time
                        </th>
                        <th
                            class="px-3 py-2 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide"
                        >
                            BP
                        </th>
                        <th
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                    ...
                    >
                        <td
                            class="px-3 py-2 text-xs text-gray-500 dark:text-slate-400 whitespace-nowrap"
                        >
                            {{ fmtDatetime(v.recorded_at) }}
                        </td>
                        <td
                            :class="`px-3 py-2 text-xs font-mono ${bpClass(v.systolic_bp, v.diastolic_bp)}`"
                        >
                            {{
                        ...
                        <td :class="`px-3 py-2 text-xs font-mono ${hrClass(v.heart_rate)}`">
                            {{ v.heart_rate ?? '-' }}
                        </td>
                        <td class="px-3 py-2 text-xs font-mono text-gray-700 dark:text-slate-300">
                            {{ v.respiratory_rate ?? '-' }}
                        </td>
                        <td class="px-3 py-2 text-xs font-mono text-gray-700 dark:text-slate-300">
                        ...
                        <td class="px-3 py-2 text-xs font-mono text-gray-700 dark:text-slate-300">
                            {{ v.weight_lbs != null ? `${v.weight_lbs} lb` : '-' }}
                        </td>
                        <td :class="`px-3 py-2 text-xs font-mono ${o2Class(v.oxygen_saturation)}`">
                        ...
                        <td class="px-3 py-2 text-xs font-mono text-gray-700 dark:text-slate-300">
                            {{ v.blood_glucose != null ? `${v.blood_glucose}` : '-' }}
                        </td>
                    </tr>
                </tbody>
            </table>
