        ...
        >
            ...
            <div class="grid grid-cols-2 gap-3 mb-3">
                <div>
                    ...
                    >
                        <option v-for="(label, key) in ASSESSMENT_LABELS" :key="key" :value="key">
                            {{ label }}
                        </option>
                    </select>
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <textarea
                        v-model="form.notes"
                        rows="2"
                        placeholder="Optional findings or observations"
                        class="w-full text-sm border border-gray-300 dark:border-slate-600 rounded-md px-2 py-1.5 bg-white dark:bg-slate-700"
                    ></textarea>
                </div>
            </div>
                ...
                <div class="flex items-center justify-between gap-2">
                    ...
                    <div class="text-right shrink-0">
                        ...
                        <div class="text-xs text-gray-400 dark:text-slate-500">
                            {{ fmtDate(assessment.assessed_at) }}
                        </div>
                    </div>
                </div>
                <div class="flex items-center gap-2 mt-1">
                    <span
                        v-if="assessment.assessed_by"
                        class="text-xs text-gray-400 dark:text-slate-500"
                    >
                        By {{ assessment.assessed_by.first_name }}
                        {{ assessment.assessed_by.last_name }}
                    </span>
                </div>
                <p v-if="assessment.notes" class="text-xs text-gray-500 dark:text-slate-400 mt-1">
                    {{ assessment.notes }}
                </p>
            </div>
        </div>
