...
const filteredProblems = computed(() =>
    filterStatus.value
        ? problems.value.filter((p) => p.status === filterStatus.value)
        : problems.value,
)
...
async function submit() {
    if (!form.value.icd10_code) {
        error.value = 'ICD-10 code is required.'
        return
    }
    saving.value = true
    error.value = ''
    ...
    <div class="p-6">
        ...
        >
            ...
            <div class="relative mb-3">
                ...
                />
                <div
                    v-if="icdMatches.length > 0"
                    class="absolute z-10 w-full mt-1 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg max-h-40 overflow-y-auto"
                >
                    <button
            ...
            >
                <input
                    v-model="form.is_primary_diagnosis"
                    type="checkbox"
        ...
        <div v-else class="space-y-1.5">
            ...
            >
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div
                        v-if="problem.onset_date"
                        class="text-xs text-gray-400 dark:text-slate-500 mt-0.5"
                    >
                        Onset: {{ fmtDate(problem.onset_date) }}
                    </div>
                    <div
                        v-if="problem.notes"
