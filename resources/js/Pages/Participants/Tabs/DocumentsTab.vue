...
interface ParticipantDocument {
  id: number; category: string; filename: string; original_filename: string
  file_size_bytes: number | null; mime_type: string | null
  description: string | null; is_active: boolean; uploaded_at: string
...
const isAdmin = computed(() => auth.value?.user.department === 'it_admin' || auth.value?.user.is_super_admin)
...
const filteredDocuments = computed(() =>
  filterCategory.value ? documents.value.filter(d => d.category === filterCategory.value) : documents.value
)
...
async function downloadDocument(doc: ParticipantDocument) {
  try {
    const res = await axios.get(`/participants/${props.participant.id}/documents/${doc.id}/download`)
  ...
  <div class="p-6">
    <div class="flex items-center justify-between mb-4 gap-2 flex-wrap">
      ...
      <div class="flex items-center gap-2">
        <select v-model="filterCategory" class="text-xs border border-gray-300 dark:border-slate-600 rounded-md px-2 py-1.5 bg-white dark:bg-slate-700">
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 flex-wrap">
            <span class="text-sm text-gray-900 dark:text-slate-100 truncate">{{ doc.original_filename }}</span>
            <span class="text-xs bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-slate-400 px-1.5 py-0.5 rounded shrink-0">{{ CATEGORY_LABELS[doc.category] ?? doc.category }}</span>
          </div>
          <div class="text-xs text-gray-400 dark:text-slate-500 flex gap-2 flex-wrap mt-0.5">
            <span>{{ formatBytes(doc.file_size_bytes) }}</span>
            <span>{{ fmtDate(doc.uploaded_at) }}</span>
            <span v-if="doc.uploaded_by">{{ doc.uploaded_by.first_name }} {{ doc.uploaded_by.last_name }}</span>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow-xl max-w-sm w-full p-5">
        ...
        <div class="space-y-3">
          ...
          <div>
            ...
            <select v-model="uploadForm.category" class="w-full text-sm border border-gray-300 dark:border-slate-600 rounded-md px-2 py-1.5 bg-white dark:bg-slate-700">
              <option v-for="(label, key) in CATEGORY_LABELS" :key="key" :value="key">{{ label }}</option>
            </select>
          </div>
          <div>
            <label class="block text-xs font-medium text-gray-700 dark:text-slate-300 mb-1">Description</label>
            <input v-model="uploadForm.description" type="text" placeholder="Optional" class="w-full text-sm border border-gray-300 dark:border-slate-600 rounded-md px-2 py-1.5 bg-white dark:bg-slate-700" />
          </div>
        </div>
