...
// Tabs/DisenrollmentTab.vue
// Disenrollment and transition plan tracking per 42 CFR 460.116.
// Manages voluntary and involuntary disenrollment with CMS notification workflow.
...
async function loadData() {
    loading.value = true
    try {
        const r = await axios.get(`/participants/${props.participantId}/disenrollment`)
    ...
    <div>
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-sm font-semibold text-gray-700 dark:text-slate-300">
                Disenrollment Records ({{ items.length }})
            </h3>
            <button
        ...
        >
            <pre class="p-4 text-xs text-gray-600 dark:text-slate-300 overflow-x-auto">{{
                JSON.stringify(items.slice(0, 3), null, 2)
            }}</pre>
            <p
                v-if="items.length > 3"
                class="px-4 py-2 text-xs text-gray-400 dark:text-slate-500 border-t border-gray-100 dark:border-slate-700"
            >
                ...and {{ items.length - 3 }} more records
            </p>
        </div>
    </div>
</template>
