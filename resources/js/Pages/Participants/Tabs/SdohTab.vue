...
const records = ref<SocialDeterminant[]>(props.sdohRecords)
        ...
        >
            ...
            <div class="grid grid-cols-2 gap-3 mb-3">
                <div>
                    ...
                    >
                        <option v-for="(label, key) in DOMAIN_LABELS" :key="key" :value="key">
                            {{ label }}
                        </option>
                    </select>
                </div>
            ...
            >
                <span
                    :class="[
                        ...
                        RISK_COLORS[record.risk_level] ?? RISK_COLORS.unknown,
                    ]"
                >
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div
                        v-if="record.notes"
                        class="text-xs text-gray-500 dark:text-slate-500 mt-0.5"
                    >
                        {{ record.notes }}
                    </div>
                    <div class="text-xs text-gray-400 dark:text-slate-500 mt-0.5">
                        {{ fmtDate(record.screened_at) }}
                        <span v-if="record.screened_by">
                            by {{ record.screened_by.first_name }}
                            {{ record.screened_by.last_name }}</span
                        >
                    </div>
                </div>
            </div>
