...
interface WoundAssessment {
  id: number; assessed_at: string; stage: string | null; length_cm: number | null
  width_cm: number | null; depth_cm: number | null; exudate: string | null
  wound_bed: string | null; periwound: string | null; notes: string | null
  assessed_by: { id: number; first_name: string; last_name: string } | null
}

interface WoundRecord {
  id: number; location: string; wound_type: string; current_stage: string | null
  status: string; onset_date: string | null; closed_date: string | null
  notes: string | null; assessments: WoundAssessment[]
}
...
const woundForm = ref({
  location: '', wound_type: 'pressure_injury', current_stage: 'stage_2',
  onset_date: new Date().toISOString().slice(0, 10), notes: '',
})

const assessmentForm = ref({
  assessed_at: new Date().toISOString().slice(0, 10),
  stage: '', length_cm: '', width_cm: '', depth_cm: '',
  exudate: '', wound_bed: '', notes: '',
})
...
async function submitWound() {
  ...
  try {
    const res = await axios.post(`/participants/${props.participant.id}/wounds`, {
      ...woundForm.value, notes: woundForm.value.notes || null,
    })
    wounds.value.unshift({ ...res.data, assessments: [] })
    showAddWound.value = false
    woundForm.value = { location: '', wound_type: 'pressure_injury', current_stage: 'stage_2', onset_date: new Date().toISOString().slice(0, 10), notes: '' }
  } catch (err: unknown) {
...
async function submitAssessment(woundId: number) {
  ...
  try {
    ...
    const res = await axios.post(`/participants/${props.participant.id}/wounds/${woundId}/assessments`, payload)
    ...
    assessmentForm.value = { assessed_at: new Date().toISOString().slice(0, 10), stage: '', length_cm: '', width_cm: '', depth_cm: '', exudate: '', wound_bed: '', notes: '' }
  } catch (err: unknown) {
    const e = err as { response?: { data?: { message?: string } } }
  ...
  <div class="p-6">
    ...
    <div v-if="showAddWound" class="bg-gray-50 dark:bg-slate-700/50 rounded-lg border border-gray-200 dark:border-slate-600 p-4 mb-4">
      ...
      <div class="grid grid-cols-2 gap-3 mb-3">
        <div>
          ...
          <input v-model="woundForm.location" type="text" placeholder="e.g. Left sacrum" class="w-full text-sm border border-gray-300 dark:border-slate-600 rounded-md px-2 py-1.5 bg-white dark:bg-slate-700" />
        </div>
        <div>
          <label class="block text-xs font-medium text-gray-700 dark:text-slate-300 mb-1">Type</label>
          <select v-model="woundForm.wound_type" class="w-full text-sm border border-gray-300 dark:border-slate-600 rounded-md px-2 py-1.5 bg-white dark:bg-slate-700">
        ...
        <div>
          <label class="block text-xs font-medium text-gray-700 dark:text-slate-300 mb-1">Initial Stage</label>
          <select v-model="woundForm.current_stage" class="w-full text-sm border border-gray-300 dark:border-slate-600 rounded-md px-2 py-1.5 bg-white dark:bg-slate-700">
        ...
        <div>
          ...
          <input v-model="woundForm.onset_date" type="date" class="w-full text-sm border border-gray-300 dark:border-slate-600 rounded-md px-2 py-1.5 bg-white dark:bg-slate-700" />
        </div>
      </div>
    ...
    <div class="space-y-3 mb-6">
      <div
        ...
        :key="wound.id"
        :class="['border rounded-lg overflow-hidden', STAGE_COLORS[wound.current_stage ?? ''] ?? 'border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800']"
      >
        <div class="flex items-start gap-3 px-4 py-3">
          <div class="flex-1">
            <div class="flex items-center gap-2 flex-wrap">
              <span class="text-sm font-semibold text-gray-900 dark:text-slate-100">{{ wound.location }}</span>
              <span class="text-xs bg-white/80 dark:bg-slate-900/60 text-gray-600 dark:text-slate-300 px-1.5 py-0.5 rounded capitalize">{{ wound.wound_type.replace('_', ' ') }}</span>
              <span v-if="wound.current_stage" class="text-xs font-medium text-gray-700 dark:text-slate-300 capitalize">{{ wound.current_stage.replace('_', ' ') }}</span>
            </div>
            <div class="text-xs text-gray-500 dark:text-slate-400 mt-0.5">
              Onset: {{ fmtDate(wound.onset_date) }} · {{ wound.assessments.length }} assessments
            </div>
          </div>
        ...
        <div v-if="addAssessmentForId === wound.id" class="border-t border-gray-200 dark:border-slate-700 bg-white/70 dark:bg-slate-900/30 px-4 py-3">
          ...
          <div class="grid grid-cols-3 gap-2 mb-2">
            <div>
              ...
              <input v-model="assessmentForm.length_cm" type="number" step="0.1" class="w-full text-xs border border-gray-300 dark:border-slate-600 rounded px-1.5 py-1 bg-white dark:bg-slate-700" />
            </div>
            <div>
              <label class="block text-xs text-gray-600 dark:text-slate-400 mb-0.5">Width (cm)</label>
              <input v-model="assessmentForm.width_cm" type="number" step="0.1" class="w-full text-xs border border-gray-300 dark:border-slate-600 rounded px-1.5 py-1 bg-white dark:bg-slate-700" />
            </div>
            <div>
              <label class="block text-xs text-gray-600 dark:text-slate-400 mb-0.5">Depth (cm)</label>
              <input v-model="assessmentForm.depth_cm" type="number" step="0.1" class="w-full text-xs border border-gray-300 dark:border-slate-600 rounded px-1.5 py-1 bg-white dark:bg-slate-700" />
            </div>
          </div>
          <div class="mb-2">
            <label class="block text-xs text-gray-600 dark:text-slate-400 mb-0.5">Notes</label>
            <input v-model="assessmentForm.notes" type="text" class="w-full text-xs border border-gray-300 dark:border-slate-600 rounded px-1.5 py-1 bg-white dark:bg-slate-700" />
          </div>
          <p v-if="assessmentError" class="text-red-600 dark:text-red-400 text-xs mb-1">{{ assessmentError }}</p>
          <div class="flex gap-2">
            <button :disabled="savingAssessment" class="text-xs px-2 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50 transition-colors" @click="submitAssessment(wound.id)">
    ...
    <div v-if="healedWounds.length > 0">
      ...
      <div class="space-y-1">
        <div v-for="wound in healedWounds" :key="wound.id" class="flex items-center gap-3 text-xs text-gray-500 dark:text-slate-400 px-4 py-2 bg-gray-50 dark:bg-slate-800/50 border border-gray-100 dark:border-slate-700 rounded-lg">
          ...
          <span class="capitalize text-gray-400 dark:text-slate-500">{{ wound.wound_type.replace('_', ' ') }}</span>
