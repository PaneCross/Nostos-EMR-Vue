...
async function submit() {
    if (!form.value.to_site_id) {
        error.value = 'Destination site is required.'
        return
    }
    saving.value = true
    error.value = ''
...
<template>
    <div class="p-6">
        ...
        <div
            ...
            class="mb-4 bg-amber-50 dark:bg-amber-950/30 border border-amber-300 dark:border-amber-700 rounded-lg p-4"
        >
            <div class="flex items-start gap-2">
                ...
                <div>
                    ...
                    <p class="text-xs text-amber-700 dark:text-amber-400 mt-0.5">
                        Transfer to <strong>{{ pendingTransfer.to_site?.name }}</strong> requested
                        on {{ fmtDate(pendingTransfer.requested_at) }}.
                        <span v-if="pendingTransfer.reason">
                            Reason: {{ pendingTransfer.reason }}</span
                        >
                    </p>
                </div>
            </div>
            ...
            >
                <div class="flex items-center gap-2 mb-1">
                    <span class="text-sm text-gray-700 dark:text-slate-300">{{
                        transfer.from_site?.name ?? 'Unknown'
                    }}</span>
                    <ArrowRightIcon
                ...
                <div class="text-xs text-gray-400 dark:text-slate-500 flex gap-3 flex-wrap">
                    ...
                    <span v-if="transfer.effective_date"
                        >Effective {{ fmtDate(transfer.effective_date) }}</span
                    >
                    <span v-if="transfer.requested_by"
                        >By {{ transfer.requested_by.first_name }}
                        {{ transfer.requested_by.last_name }}</span
                    >
                    <span v-if="transfer.approved_by"
                        >Approved by {{ transfer.approved_by.first_name }}
                        {{ transfer.approved_by.last_name }}</span
                    >
                </div>
                <p v-if="transfer.reason" class="text-xs text-gray-500 dark:text-slate-400 mt-1">
                    {{ transfer.reason }}
                </p>
            </div>
        </div>
