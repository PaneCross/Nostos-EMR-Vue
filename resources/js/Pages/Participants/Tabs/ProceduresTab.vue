...
async function submit() {
    if (!form.value.procedure_name.trim()) {
        error.value = 'Procedure name is required.'
        return
    }
    saving.value = true
    error.value = ''
            ...
            >
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <span
                            v-if="proc.cpt_code"
                            class="font-mono text-xs bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-300 px-1.5 py-0.5 rounded"
                            >{{ proc.cpt_code }}</span
                        >
                        <span
                            :class="[
                                'text-xs px-1.5 py-0.5 rounded',
                                SOURCE_BADGES[proc.source] ?? '',
                            ]"
                            >{{ SOURCE_LABELS[proc.source] ?? proc.source }}</span
                        >
                    </div>
                    <div
                    ...
                    <p v-if="proc.notes" class="text-xs text-gray-500 dark:text-slate-400 mt-1">
                        {{ proc.notes }}
                    </p>
                </div>
            </div>
