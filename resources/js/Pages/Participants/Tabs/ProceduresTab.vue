...
onMounted(async () => {
  try {
    const r = await axios.get(`/participants/${props.participant.id}/procedures`)
    procedures.value = r.data
  } catch {
    loadError.value = 'Failed to load procedures.'
  } finally {
    loading.value = false
  }
})
...
<template>
  <div class="p-6">
    <div v-if="loading" class="py-12 text-center text-gray-400 dark:text-slate-500 text-sm">Loading procedures...</div>
