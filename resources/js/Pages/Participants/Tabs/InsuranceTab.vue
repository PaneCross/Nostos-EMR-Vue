    ...
    try {
        if (editingId.value) {
            const res = await axios.put(
                `/participants/${props.participant.id}/insurance/${editingId.value}`,
                payload,
            )
                ...
                <div class="flex items-start gap-3">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            <span
                                class="text-xs bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-300 px-1.5 py-0.5 rounded"
                                >{{
                                    INSURANCE_TYPE_LABELS[cov.insurance_type] ?? cov.insurance_type
                                }}</span
                            >
                            <span
                                v-if="cov.is_primary"
                            ...
                            <span
                                :class="[
                                    ...
                                    ELIGIBILITY_COLORS[cov.eligibility_status] ?? '',
                                ]"
                                >{{ cov.eligibility_status }}</span
                            >
                        </div>
                        <div
                            class="text-xs text-gray-500 dark:text-slate-400 mt-1 flex gap-3 flex-wrap"
                        >
                        ...
                        <div
                            class="text-xs text-gray-400 dark:text-slate-500 mt-0.5 flex gap-3 flex-wrap"
                        >
                            <span v-if="cov.effective_date"
                                >Effective: {{ fmtDate(cov.effective_date) }}</span
                            >
                            <span v-if="cov.termination_date"
                                >Ends: {{ fmtDate(cov.termination_date) }}</span
                            >
                        </div>
                        <p
                            v-if="cov.notes"
        ...
        >
            ...
            >
                <h3 class="text-sm font-semibold text-gray-900 dark:text-slate-100 mb-3">
                    {{ editingId ? 'Edit Coverage' : 'Add Coverage' }}
                </h3>
                <div class="grid grid-cols-2 gap-3 mb-3">
                    <div>
                        ...
                        >
                            <option
                                v-for="(label, key) in INSURANCE_TYPE_LABELS"
                                :key="key"
                                :value="key"
                            >
                                {{ label }}
                    ...
                    <div>
                        ...
                        >
                        <select
                            v-model="form.eligibility_status"
                            class="w-full text-sm border border-gray-300 dark:border-slate-600 rounded-md px-2 py-1.5 bg-white dark:bg-slate-700"
                        >
