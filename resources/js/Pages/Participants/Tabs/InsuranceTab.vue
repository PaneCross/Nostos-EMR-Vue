...
<template>
    <div>
        <h3 class="text-sm font-semibold text-gray-700 dark:text-slate-300 mb-4">
            Insurance Coverage ({{ insurances.length }})
        </h3>
        <p
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            >
                <div class="flex items-center justify-between mb-2">
                    <span class="font-semibold text-sm text-gray-900 dark:text-slate-100">{{
                        PAYER_LABELS[ins.payer_type] ?? ins.payer_type
                    }}</span>
                    <span
                        :class="`text-xs px-2 py-0.5 rounded-full font-medium ${ins.is_active ? 'bg-green-100 dark:bg-green-900/60 text-green-700 dark:text-green-300' : 'bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-slate-400'}`"
                    >
                ...
                <dl class="grid grid-cols-2 gap-2 text-sm">
                    <div v-if="ins.member_id">
                        ...
                        <dd class="mt-0.5 text-sm text-gray-900 dark:text-slate-100 font-mono">
                            {{ ins.member_id }}
                        </dd>
                    </div>
                    ...
                    <div v-if="ins.effective_date">
                        ...
                        <dd class="mt-0.5 text-sm text-gray-900 dark:text-slate-100">
                            {{ new Date(ins.effective_date).toLocaleDateString('en-US') }}
                        </dd>
                    </div>
                    <div v-if="ins.term_date">
                        ...
                        <dd class="mt-0.5 text-sm text-gray-900 dark:text-slate-100">
                            {{ new Date(ins.term_date).toLocaleDateString('en-US') }}
                        </dd>
                    </div>
                </dl>
            </div>
