...
const FLAG_CONFIG: Record<string, { label: string; classes: string; dotColor: string }> = {
    wheelchair: {
        label: 'Wheelchair',
        classes:
            'bg-blue-100 dark:bg-blue-900/60 text-blue-800 dark:text-blue-300 ring-blue-600/20',
...
const filtered = computed(() => {
    return props.participants.filter((p) => {
        ...
        if (
            flagFilter.value &&
            flagFilter.value !== 'none' &&
            !p.flags.some((f) => f.flag_type === flagFilter.value)
        )
            return false
        if (search.value) {
...
const statChips = computed(() => [
    {
        label: 'Active Census',
        count: props.stats.total_active,
        color: 'bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300',
        filter: '',
    },
    {
        label: 'Wheelchair',
        count: props.stats.needs_wheelchair,
        color: 'bg-blue-50 dark:bg-blue-950/60 border-blue-200 dark:border-blue-800 text-blue-800 dark:text-blue-300',
        filter: 'wheelchair',
    },
    {
        label: 'Stretcher',
        count: props.stats.needs_stretcher,
        color: 'bg-orange-50 border-orange-200 text-orange-800',
        filter: 'stretcher',
    },
    {
        label: 'Oxygen',
        count: props.stats.needs_oxygen,
        color: 'bg-teal-50 dark:bg-teal-950/60 border-teal-200 text-teal-800 dark:text-teal-300',
        filter: 'oxygen',
    },
    {
        label: 'Behavioral',
        count: props.stats.has_behavioral,
        color: 'bg-red-50 dark:bg-red-950/60 border-red-200 dark:border-red-800 text-red-800 dark:text-red-300',
        filter: 'behavioral',
    },
    {
        label: 'No Flags',
        count: props.stats.no_flags,
        color: 'bg-green-50 dark:bg-green-950/60 border-green-200 dark:border-green-800 text-green-800 dark:text-green-300',
        filter: 'none',
    },
])
...
<template>
    <AppShell :breadcrumbs="[{ label: 'Transportation' }, { label: 'Dashboard' }]">
        ...
        <div
            class="mb-5 flex items-start gap-3 rounded-xl border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-950/40 px-4 py-3"
        >
            ...
            <p class="text-sm text-amber-800 dark:text-amber-300">
                ...
                Transport integration is pending the Nostos transport deployment. Trip scheduling,
                dispatch map, and route management will be available once the live bridge is
                connected.
            </p>
        </div>
        ...
        <div class="flex items-center justify-between mb-5">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                    Active participant transport needs, mobility equipment, behavioral flags. Click
                    a row to view profile.
                </p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-5">
            <button
                ...
                :class="[
                    ...
                    chip.color,
                    flagFilter === chip.filter && chip.filter !== ''
        ...
        >
            ...
            <div>
                <p class="text-sm font-semibold text-red-800 dark:text-red-300">
                    {{ behavioralParticipants.length }} participant{{
                        behavioralParticipants.length !== 1 ? 's' : ''
                    }}
        ...
        >
            ...
            <table
                v-else
                class="min-w-full divide-y divide-slate-100 dark:divide-slate-700 text-sm"
            >
                <thead class="bg-slate-50 dark:bg-slate-900">
                    <tr>
                        <th
                            class="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide"
                        >
                            Participant
                        </th>
                        <th
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                    ...
                    >
                        ...
                        <td class="px-4 py-3">
                            <span v-if="p.flags.length === 0" class="text-slate-300 text-xs"
                                >None</span
                            >
                            <div v-else class="flex flex-wrap gap-1">
                                <span
                                    ...
                                    :class="[
                                        ...
                                        FLAG_CONFIG[flag.flag_type]?.classes ?? '',
                                    ]"
                                    :title="
                                        flag.description ??
                                        FLAG_CONFIG[flag.flag_type]?.label ??
                                        flag.flag_type
                                    "
                                >
                                    <span
                                        :class="[
                                            ...
                                            SEVERITY_CLASSES[flag.severity] ?? 'bg-slate-400',
                                        ]"
                                    ></span>
        ...
        <p class="mt-3 text-xs text-slate-400">
            Transport flags are synced from participant profiles. Trip scheduling, dispatch map, and
            route management are coming in a future phase.
        </p>
    </AppShell>
</template>
