...
const STATUS_CONFIG: Record<string, { label: string; badgeClass: string }> = {
    requested:  { label: 'Requested',  badgeClass: 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400' },
    scheduled:  { label: 'Scheduled',  badgeClass: 'bg-blue-100 dark:bg-blue-900/60 text-blue-700 dark:text-blue-300' },
    dispatched: { label: 'Dispatched', badgeClass: 'bg-indigo-100 dark:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300' },
    en_route:   { label: 'En Route',   badgeClass: 'bg-amber-100 dark:bg-amber-900/60 text-amber-700 dark:text-amber-300' },
    arrived:    { label: 'Arrived',    badgeClass: 'bg-teal-100 text-teal-700 dark:text-teal-300' },
    completed:  { label: 'Completed',  badgeClass: 'bg-green-100 dark:bg-green-900/60 text-green-700 dark:text-green-300' },
    no_show:    { label: 'No Show',    badgeClass: 'bg-red-100 dark:bg-red-900/60 text-red-700 dark:text-red-300' },
...
<template>
    <AppShell :breadcrumbs="[{ label: 'Transportation' }, { label: 'Manifest' }]">
        ...
        <div class="flex items-center justify-between mb-5">
            <div>
                <h1 class="text-xl font-bold text-slate-900 dark:text-slate-100">Transport Manifest</h1>
        ...
        <div class="flex items-start gap-3 rounded-xl border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-950/40 px-5 py-4 mb-4">
            <ExclamationTriangleIcon class="w-5 h-5 text-amber-500 shrink-0 mt-0.5" aria-hidden="true" />
            <div>
                <p class="text-sm font-semibold text-amber-800 dark:text-amber-200">Nostos Integration Pending</p>
                <p class="text-sm text-amber-700 dark:text-amber-300 mt-0.5">
                    ...
                    The run sheet will populate automatically once the live bridge is connected.
                    EMR-side transport requests are stored and ready for sync.
                </p>
            </div>
        </div>
        ...
        <template v-if="activeTab === 'runsheet'">
            <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
                <div class="px-6 py-12 text-center">
                    <TruckIcon class="w-12 h-12 text-slate-300 dark:text-slate-600 mx-auto mb-4" aria-hidden="true" />
                    ...
                    <p class="text-xs text-slate-400 dark:text-slate-500 max-w-md mx-auto">
                        Trip data will appear here once the Nostos transport integration is connected.
                        Transport requests submitted via the EMR are queued and ready.
                    </p>
                </div>
            </div>
        </template>
        ...
        <template v-if="activeTab === 'addon'">
            <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
                <div class="px-6 py-12 text-center">
                    <ClipboardDocumentListIcon class="w-12 h-12 text-slate-300 dark:text-slate-600 mx-auto mb-4" aria-hidden="true" />
                    <p class="text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">
                        No pending add-on requests
                    </p>
                    <p class="text-xs text-slate-400 dark:text-slate-500 max-w-md mx-auto">
                        Add-on trip requests from clinical staff will queue here for transport team approval
                        once the Nostos transport integration is live.
                    </p>
                </div>
            </div>
        </template>
        ...
        <p class="mt-3 text-xs text-slate-400">
            Export PDF and real-time status updates will be available after transport integration is connected.
        </p>
    </AppShell>
</template>
