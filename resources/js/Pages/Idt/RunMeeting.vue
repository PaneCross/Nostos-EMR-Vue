...
watch(
    ...
    (newId) => {
        ...
        if (review) {
            ...
            actionItems.value = review.action_items
                ? JSON.parse(JSON.stringify(review.action_items))
                : []
        }
...
<template>
    <AppShell>
        <Head :title="`IDT Meeting - ${props.meeting.meeting_date}`" />

        <div class="flex flex-col h-full">
            <!-- Top bar -->
            <div
                class="px-6 py-4 border-b border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex items-center justify-between gap-4 shrink-0"
            >
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-base font-bold text-gray-900 dark:text-slate-100">
                            {{
                                TYPE_LABELS[props.meeting.meeting_type] ??
                                props.meeting.meeting_type
                            }}
                        </h1>
                        <p class="text-xs text-gray-500 dark:text-slate-400">
                            {{ props.meeting.meeting_date }}
                            <template v-if="props.meeting.meeting_time">
                                · {{ fmt12h(props.meeting.meeting_time) }}</template
                            >
                            <template v-if="props.meeting.facilitator">
                                · {{ props.meeting.facilitator.first_name }}
                                {{ props.meeting.facilitator.last_name }}
                            </template>
            ...
            <p
                v-if="completeError"
                class="px-6 py-2 text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/60"
            >
                {{ completeError }}
            </p>
            ...
            <div class="flex flex-1 min-h-0 overflow-hidden">
                ...
                <aside
                    class="w-60 shrink-0 border-r border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900 overflow-y-auto"
                >
                    <div class="p-3 space-y-1">
                        <p
                            class="text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider px-2 py-1"
                        >
                            Participants
                        </p>
                        <button
                        ...
                        >
                            ...
                            <span class="truncate font-medium">
                                {{ review.participant.last_name }},
                                {{ review.participant.first_name }}
                            </span>
                        </button>

                        <p
                            v-if="sortedReviews.length === 0"
                            class="text-xs text-gray-400 dark:text-slate-500 px-2 py-2"
                        >
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                <h2 class="text-lg font-bold text-gray-900 dark:text-slate-100">
                                    {{ selectedReview.participant.last_name }},
                                    {{ selectedReview.participant.first_name }}
                                </h2>
                                <p
                                    class="text-xs text-gray-500 dark:text-slate-400 font-mono mt-0.5"
                                >
