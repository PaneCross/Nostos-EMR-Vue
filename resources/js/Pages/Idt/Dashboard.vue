  ...
  <AppShell>
    ...
    <div class="px-6 py-6 space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-xl font-bold text-gray-900 dark:text-slate-100">IDT Dashboard</h1>
      ...
      <!-- Today's Meetings -->
      <section>
        <h2 class="text-sm font-semibold text-gray-700 dark:text-slate-300 uppercase tracking-wider mb-3">
          Today's Meetings
        </h2>

        <div v-if="props.todayMeetings.length === 0"
          class="rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-6 py-8 text-center"
        >
          <CalendarIcon class="w-8 h-8 mx-auto text-gray-300 dark:text-slate-600 mb-2" aria-hidden="true" />
          <p class="text-sm text-gray-500 dark:text-slate-400">No meetings scheduled for today.</p>
          <button
        ...
        <div v-else class="space-y-3">
          ...
          >
            <div class="flex items-start justify-between gap-3 mb-2">
              <div>
                <div class="flex items-center gap-2">
                  <span class="font-semibold text-sm text-gray-800 dark:text-slate-200">
                ...
                <p class="text-xs text-gray-500 dark:text-slate-400 mt-0.5">
                  ...
                  <template v-if="meeting.meeting_time"> · {{ fmt12h(meeting.meeting_time) }}</template>
            ...
            <div v-if="totalReviews(meeting) > 0" class="mt-2">
              <div class="flex items-center gap-2 text-xs text-gray-600 dark:text-slate-400 mb-1">
                <UsersIcon class="w-3.5 h-3.5" aria-hidden="true" />
                {{ reviewedCount(meeting) }} / {{ totalReviews(meeting) }} participants reviewed
              </div>
              <div class="h-1.5 rounded-full bg-gray-200 dark:bg-slate-700 overflow-hidden">
                <div
                  class="h-full bg-green-500 rounded-full transition-all"
                  :style="{ width: `${totalReviews(meeting) > 0 ? (reviewedCount(meeting) / totalReviews(meeting)) * 100 : 0}%` }"
                />
              </div>
            </div>
          </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <section>
          ...
          <div v-if="props.upcomingMeetings.length === 0"
            ...
            >
              <div>
                ...
                <p class="text-xs text-gray-500 dark:text-slate-400">
                  {{ fmtDate(meeting.meeting_date) }}<template v-if="meeting.meeting_time"> · {{ fmt12h(meeting.meeting_time) }}</template>
                </p>
              </div>
              <a
                :href="`/idt/meetings/${meeting.id}`"
        ...
        <section>
          ...
          <div v-if="props.recentMeetings.length === 0"
            class="rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-6 text-center text-sm text-gray-500 dark:text-slate-400"
          >
          ...
          <div v-else class="space-y-2">
            ...
            >
              ...
              <div class="flex items-center gap-1.5">
                <CheckCircleIcon class="w-3.5 h-3.5 text-green-500" aria-hidden="true" />
                <a
                  :href="`/idt/meetings/${meeting.id}`"
                  class="text-xs text-blue-600 dark:text-blue-400 hover:underline font-medium"
                >
    ...
    >
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-md" role="dialog" aria-modal="true">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between">
          <h2 class="font-semibold text-gray-800 dark:text-slate-200">Schedule IDT Meeting</h2>
          <button
        ...
        <div class="px-6 py-5 space-y-4">
          <p v-if="scheduleError" class="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/60 rounded-lg px-3 py-2">
            {{ scheduleError }}
          </p>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-xs font-medium text-gray-700 dark:text-slate-300 mb-1">Date</label>
              <input
            ...
            <div>
              <label class="block text-xs font-medium text-gray-700 dark:text-slate-300 mb-1">Time</label>
              <input
