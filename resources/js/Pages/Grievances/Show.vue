        ...
        <div class="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    >
                        <div class="flex flex-wrap items-start justify-between gap-4 mb-4">
                            <div>
                                <p class="text-xs font-mono text-gray-500 dark:text-slate-400 mb-1">
                                    {{ props.grievance.reference_number }}
                                </p>
                                <h1 class="text-xl font-bold text-gray-900 dark:text-slate-100">
                                    {{ props.grievance.category_label }}
                                </h1>
                            </div>
                            <div class="flex gap-2 flex-wrap">
                                ...
                                <span
                                    :class="[
                                        ...
                                        statusClass(props.grievance.status),
                                    ]"
                                >
                                    {{
                                        props.statuses[props.grievance.status] ??
                                        props.grievance.status.replace(/_/g, ' ')
                                    }}
                                </span>
                                <!-- CMS flag -->
                                <span
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
                            ...
                            <div v-if="props.grievance.resolution_date">
                                ...
                                <p class="font-medium text-gray-800 dark:text-slate-200">
                                    {{ fmt(props.grievance.resolution_date) }}
                                </p>
                            </div>
                            <div v-if="props.grievance.notified_at">
                                ...
                                <p class="font-medium text-gray-800 dark:text-slate-200">
                                    {{ fmt(props.grievance.notified_at) }}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    >
                        <h2
                            class="text-sm font-semibold text-gray-700 dark:text-slate-300 uppercase mb-3 flex items-center gap-2"
                        >
                            <DocumentTextIcon class="w-4 h-4" />
                            Grievance Description
                        </h2>
                        <p
                            class="text-sm text-gray-700 dark:text-slate-300 whitespace-pre-wrap leading-relaxed"
                        >
                            {{ props.grievance.description }}
                        </p>
                    </div>
                    ...
                    >
                        <h2
                            class="text-sm font-semibold text-gray-700 dark:text-slate-300 uppercase mb-3"
                        >
                            Investigation Notes
                        </h2>
                        <p
                            class="text-sm text-gray-700 dark:text-slate-300 whitespace-pre-wrap leading-relaxed"
                        >
                            {{ props.grievance.investigation_notes }}
                        </p>
                    </div>
                    ...
                    <div
                        v-if="
                            props.grievance.status === 'resolved' && props.grievance.resolution_text
                        "
                        class="bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-800 p-6"
                    >
                        <h2
                            class="text-sm font-semibold text-green-700 dark:text-green-300 uppercase mb-3 flex items-center gap-2"
                        >
                            <CheckCircleIcon class="w-4 h-4" />
                            Resolution
                        </h2>
                        <p
                            class="text-sm text-green-800 dark:text-green-200 whitespace-pre-wrap leading-relaxed"
                        >
                            {{ props.grievance.resolution_text }}
                        </p>
                    </div>
                    ...
                    <div
                        v-if="
                            props.grievance.status === 'escalated' &&
                            props.grievance.escalation_reason
                        "
                        class="bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-800 p-6"
                    >
                        <h2
                            class="text-sm font-semibold text-red-700 dark:text-red-300 uppercase mb-3 flex items-center gap-2"
                        >
                            <ArrowUpIcon class="w-4 h-4" />
                            Escalation
                        </h2>
                        <p
                            class="text-sm text-red-800 dark:text-red-200 whitespace-pre-wrap leading-relaxed"
                        >
                            {{ props.grievance.escalation_reason }}
                        </p>
                        <div v-if="props.grievance.escalated_to_name" class="mt-2">
                            <span
                                class="inline-block px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300"
                            >
                ...
                <div class="space-y-5">
                    ...
                    >
                        <h2
                            class="text-sm font-semibold text-gray-700 dark:text-slate-300 uppercase"
                        >
                            Actions
                        </h2>
                        ...
                        <div>
                            ...
                            <div
                                v-else
                                class="space-y-2 border border-green-200 dark:border-green-800 rounded-lg p-4 bg-green-50 dark:bg-green-900/20"
                            >
                                <label
                                ...
                                <div class="flex gap-2">
                                    <button
                                        :disabled="actionLoading"
                                        class="flex-1 px-3 py-1.5 text-xs rounded-lg bg-green-600 text-white hover:bg-green-700 disabled:opacity-50 transition"
                                        @click="submitResolve()"
                                    >
                                        Confirm
                                    </button>
                                    <button
                                        class="px-3 py-1.5 text-xs rounded-lg border border-gray-300 dark:border-slate-600 text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700 transition"
                        ...
                        <div v-if="['open', 'under_review'].includes(props.grievance.status)">
                            ...
                            <div
                                v-else
                                class="space-y-2 border border-purple-200 dark:border-purple-800 rounded-lg p-4 bg-purple-50 dark:bg-purple-900/20"
                            >
                                <label
                                ...
                                >
                                    ...
                                    <option v-for="s in escalationStaff" :key="s.id" :value="s.id">
                                        {{ s.name }} — {{ s.department }}
                                    </option>
                                </select>
                                <div class="flex gap-2">
                                    <button
                                        :disabled="actionLoading"
                                        class="flex-1 px-3 py-1.5 text-xs rounded-lg bg-purple-600 text-white hover:bg-purple-700 disabled:opacity-50 transition"
                        ...
                        <div>
                            ...
                            <div
                                v-else
                                class="border border-slate-200 dark:border-slate-600 rounded-lg p-4 bg-slate-50 dark:bg-slate-700/50 text-sm"
                            >
                                <p class="text-gray-700 dark:text-slate-300 mb-3">
                                    Confirm withdrawal of this grievance?
                                </p>
                                <div class="flex gap-2">
                                    <button
                                        :disabled="actionLoading"
                                        class="flex-1 px-3 py-1.5 text-xs rounded-lg bg-slate-600 text-white hover:bg-slate-700 disabled:opacity-50 transition"
                    ...
                    >
                        <h2
                            class="text-sm font-semibold text-gray-700 dark:text-slate-300 uppercase flex items-center gap-2"
                        >
                        ...
                        <div class="flex items-center gap-2 text-sm">
                            <span
                                :class="[
                                    'font-medium',
                                    props.grievance.cms_reportable
                        ...
                        <div
                            v-if="props.grievance.cms_submitted_at"
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <button
                                v-if="
                                    props.grievance.cms_reportable &&
                                    !props.grievance.cms_submitted_at
                                "
                                :disabled="actionLoading"
                    ...
                    >
                        ...
                        <div
                            v-if="props.grievance.notified_at"
                            class="text-sm text-teal-600 dark:text-teal-400 flex items-center gap-1"
                        >
                            <CheckCircleIcon class="w-4 h-4" />
                            Notified {{ fmt(props.grievance.notified_at) }} via
                            {{ props.grievance.notification_method }}
                        </div>
                        ...
                        <div v-else>
                            ...
                            <div v-else class="space-y-2">
                                ...
                                >
                                    <option
                                        v-for="(label, key) in props.notificationMethods"
                                        :key="key"
                                        :value="key"
                                    >
                                        {{ label }}
                                ...
                                <div class="flex gap-2">
                                    <button
                                        :disabled="actionLoading"
                                        class="flex-1 px-3 py-1.5 text-xs rounded-lg bg-teal-600 text-white hover:bg-teal-700 disabled:opacity-50 transition"
                                        @click="submitNotify()"
                                    >
                                        Record Notification
                                    </button>
                    ...
                    >
                        <h2
                            class="text-sm font-semibold text-gray-700 dark:text-slate-300 uppercase mb-4 flex items-center gap-2"
                        >
                        ...
                        <ol
                            v-else
                            class="relative border-l border-gray-200 dark:border-slate-700 space-y-5 ml-2"
                        >
                            <li v-for="entry in props.activity" :key="entry.id" class="ml-4">
                                <span
                                    :class="[
                                        'absolute w-3 h-3 rounded-full -left-1.5 border-2 border-white dark:border-slate-800',
                                        activityDot(entry.action),
                                    ]"
                                ></span>
