...
const encounterItems = computed<ActionItem[]>(() =>
    encounters.value.map((e) => ({
        ...
        sublabel:
            [e.service_date, e.procedure_code ?? '-'].filter(Boolean).join(' | ') || undefined,
        badge: e.submission_status ?? '-',
        badgeColor:
            e.submission_status === 'accepted'
...
const authorizationItems = computed<ActionItem[]>(() =>
    authorizations.value.map((a) => ({
        ...
        sublabel:
            [a.authorized_start, a.authorized_end ? `to ${a.authorized_end}` : null]
                .filter(Boolean)
                .join(' ') || undefined,
        badge:
            a.status === 'active'
...
<template>
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <ActionWidget
            ...
            empty-message="No recent encounters."
            view-all-href="/finance/encounters"
            :loading="loading"
        />
        ...
        <ActionWidget
            ...
            empty-message="No authorization issues found."
            view-all-href="/finance/dashboard"
            :loading="loading"
        />
