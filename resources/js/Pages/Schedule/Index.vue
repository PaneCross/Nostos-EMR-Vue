...
const COLOR_CLASS_MAP: Record<string, { bg: string; border: string; text: string }> = {
    ...
    rose: {
        bg: 'bg-rose-100 dark:bg-rose-800/50',
    ...
    cyan: {
        bg: 'bg-cyan-100 dark:bg-cyan-800/50',
        border: 'border-cyan-400 dark:border-cyan-500',
        text: 'text-cyan-800 dark:text-cyan-200',
    },
    lime: {
        bg: 'bg-lime-100 dark:bg-lime-800/50',
...
async function markComplete() {
    ...
    try {
        const res = await axios.patch(
            `/participants/${appt.participant.id}/appointments/${appt.id}/complete`,
        )
...
async function markNoShow() {
    ...
    try {
        const res = await axios.patch(
            `/participants/${appt.participant.id}/appointments/${appt.id}/no-show`,
        )
        handleStatusChange(res.data)
    } catch {
...
async function submitBooking() {
    if (!selectedParticipant.value) {
        bookingError.value = 'Please select a participant first.'
        return
    }
...
<template>
    <AppShell>
        ...
        <!-- Page header -->
        <div
            class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800"
        >
            <div class="flex items-center gap-3">
                ...
                <span v-if="loading" class="text-xs text-gray-400 dark:text-slate-500 animate-pulse"
                    >Loading...</span
                >
            </div>
        ...
        <div class="flex flex-1 overflow-hidden">
            <!-- Time gutter -->
            <div
                class="w-14 flex-shrink-0 border-r border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/60"
            >
                <!-- Spacer to align with day header -->
                <div class="h-12 border-b border-gray-200 dark:border-slate-700"></div>
                <!-- Hour labels -->
                <div
                    v-for="h in HOURS"
                    :key="h"
                ...
                <div class="flex min-w-max">
                   ...
                   pointer-events-none so it never blocks appointment clicks. -->
                            <div
                                ...
                                :style="{ top: `${nowPct}%` }"
                            >
                                <div class="w-full border-t-2 border-red-400"></div>
                                <div
                            ...
                            <button
                                ...
                                :class="[
                                    ...
                                    appt.status === 'cancelled' || appt.status === 'no_show'
                                        ...
                                        : '',
                                    (
                                        COLOR_CLASS_MAP[props.typeColors[appt.appointment_type]] ??
                            ...
                            >
                                <p class="text-xs font-semibold leading-tight truncate">
                                    {{
                                        props.typeLabels[appt.appointment_type] ||
                                        appt.appointment_type
                                    }}
                                </p>
                                <p
                                    v-if="appt.participant"
                                    class="text-xs leading-tight truncate opacity-80"
                                >
                                    {{ appt.participant.first_name }}
                                    {{ appt.participant.last_name }}
                                </p>
                                <TruckIcon
                                    v-if="appt.transport_required"
                                    class="w-3 h-3"
                                    aria-hidden="true"
                                />
                            </button>
        ...
        >
            ...
            <div
                :class="[
                    ...
                    (
                        COLOR_CLASS_MAP[props.typeColors[selectedAppt.appointment_type]] ??
                        COLOR_CLASS_MAP.gray
                    ).bg,
                ]"
            >
                    ...
                    <div>
                        <span
                            :class="[
                                ...
                                (
                                    COLOR_CLASS_MAP[
                                        props.typeColors[selectedAppt.appointment_type]
                                    ] ?? COLOR_CLASS_MAP.gray
                                ).text,
                                (
                                    COLOR_CLASS_MAP[
                        ...
                        >
                            {{
                                props.typeLabels[selectedAppt.appointment_type] ||
                        ...
                        <h3 class="text-base font-semibold text-gray-900 dark:text-slate-100">
                            <template v-if="selectedAppt.participant">
                                {{ selectedAppt.participant.first_name }}
                                {{ selectedAppt.participant.last_name }}
                            </template>
                            <template v-else>Unknown</template>
                        </h3>
                        <p
                            v-if="selectedAppt.participant"
            ...
            <div class="flex-1 overflow-y-auto p-5 space-y-4">
                <div class="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                    ...
                    <div>
                        ...
                        <p
                            class="font-medium mt-0.5 text-gray-900 dark:text-slate-100 flex items-center gap-1"
                        >
                            <template v-if="selectedAppt.transport_required">
                                <TruckIcon class="w-4 h-4 inline-block" aria-hidden="true" />
                                Required
                            </template>
                    ...
                    <div class="col-span-2">
                        ...
                        <p class="font-medium mt-0.5 text-gray-900 dark:text-slate-100">
                            {{
                                new Date(selectedAppt.scheduled_start).toLocaleDateString('en-US', {
                                    weekday: 'short',
                                    month: 'short',
                                    day: 'numeric',
                                })
                            ...
                            &middot;
                            {{
                                new Date(selectedAppt.scheduled_start).toLocaleTimeString('en-US', {
                                    hour: 'numeric',
                                    minute: '2-digit',
                                })
                            }}
                            -
                            {{
                                new Date(selectedAppt.scheduled_end).toLocaleTimeString('en-US', {
                                    hour: 'numeric',
                                    minute: '2-digit',
                                })
                            }}
                ...
                <p
                    v-if="panelError"
                    class="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/60 rounded p-2"
                >
                    {{ panelError }}
                </p>
        ...
        >
            <div
                class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-lg"
                role="dialog"
                aria-modal="true"
            >
                <!-- Modal header -->
                ...
                <div class="p-6 space-y-4">
                            ...
                            >
                                <li v-for="p in participantResults" :key="p.id">
                                    ...
                                    >
                                        ...
                                        >
                                        <span
                                            class="text-gray-500 dark:text-slate-400 ml-2 text-xs"
                                            >{{ p.mrn }}</span
                                        >
                                    </button>
                                </li>
                    ...
                    <template v-else-if="bookingStep === 2 && selectedParticipant">
                        ...
                        <div
                            class="flex items-center gap-2 p-2 bg-blue-50 dark:bg-blue-950/60 rounded-lg"
                        >
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option v-for="l in props.locations" :key="l.id" :value="l.id">
                                    {{ l.name }}
                                </option>
                            </select>
                        </div>
                        ...
                        <div
                            class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-slate-600"
                        >
                            <input
                            ...
                            />
                            <label
                                for="transport_required"
                                class="text-sm font-medium text-gray-700 dark:text-slate-300"
                            >
                                Transport Required
                                <span
                                    class="block text-xs text-gray-500 dark:text-slate-400 font-normal"
                                    >Check if participant needs a ride</span
                                >
                            </label>
                        </div>
