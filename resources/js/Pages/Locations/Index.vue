...
import { PlusIcon, PencilIcon, ArchiveBoxIcon, ExclamationTriangleIcon } from '@heroicons/vue/24/outline'
...
function typeColor(locationType: string): string {
  return TYPE_COLORS[locationType] ?? 'bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-300 ring-slate-600/20'
}
...
function handleSave() {
  ...
  if (modal.value === 'create') {
    router.post('/locations', payload, {
      onSuccess: () => { saving.value = false; closeModal() },
      onError:   (errs) => { errors.value = errs as typeof errors.value; saving.value = false },
...
function handleArchive(loc: Location) {
  router.delete(`/locations/${loc.id}`, {
    onFinish: () => { confirmArchive.value = null },
  })
}
</script>
  ...
  <AppShell>
    ...
    <div class="flex flex-wrap items-center gap-3 mb-4">
      ...
      >
        ...
        <option v-for="(label, key) in location_types" :key="key" :value="key">{{ label }}</option>
      </select>

      <div class="flex rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 overflow-hidden shadow-sm text-sm">
        <button
          v-for="opt in ['active', 'inactive', 'all'] as const"
          :key="opt"
          @click="filterActive = opt"
          class="px-3 py-1.5 capitalize transition-colors"
          :class="filterActive === opt
    ...
    <div class="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm">
      ...
      <table v-else class="w-full text-sm">
        <thead class="border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">
          <tr>
            ...
            <th v-if="can_write" class="px-4 py-3" />
          </tr>
        </thead>
        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
          ...
          >
            ...
            <td class="px-4 py-3 text-slate-600 dark:text-slate-400">{{ loc.phone ?? '-' }}</td>
            <td class="px-4 py-3 text-slate-600 dark:text-slate-400">{{ loc.contact_name ?? '-' }}</td>
            <td class="px-4 py-3">
              <span v-if="loc.deleted_at" class="inline-flex items-center rounded-full bg-slate-100 dark:bg-slate-800 px-2 py-0.5 text-xs font-medium text-slate-500 dark:text-slate-400">Archived</span>
              <span v-else-if="loc.is_active" class="inline-flex items-center rounded-full bg-green-50 dark:bg-green-950/60 px-2 py-0.5 text-xs font-medium text-green-700 dark:text-green-300">Active</span>
              <span v-else class="inline-flex items-center rounded-full bg-amber-50 dark:bg-amber-950/60 px-2 py-0.5 text-xs font-medium text-amber-700 dark:text-amber-300">Inactive</span>
            </td>
    ...
    <!-- Create / Edit Modal -->
    <div v-if="modal !== 'none'" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
        ...
        <div class="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                :class="errors.name ? 'border-red-400' : 'border-slate-300 dark:border-slate-600'"
          ...
          <div class="grid grid-cols-3 gap-3">
            <div>
              ...
              <input v-model="form.city" class="w-full rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            </div>
            <div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input v-model="form.phone" placeholder="(555) 555-0100" class="w-full rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            </div>
            <div>
              <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Contact Name</label>
              <input v-model="form.contact_name" placeholder="Jane Smith" class="w-full rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            </div>
          </div>
          ...
          <label class="flex items-center gap-2 cursor-pointer select-none">
            ...
            />
            <span class="text-sm text-slate-700 dark:text-slate-300">Active (visible in scheduling dropdowns)</span>
          </label>
        </div>
    ...
    <!-- Archive Confirm -->
    <div v-if="confirmArchive" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div class="w-full max-w-sm rounded-2xl bg-white dark:bg-slate-800 shadow-xl p-6 text-center">
        ...
        <h3 class="text-base font-semibold text-slate-900 dark:text-slate-100 mb-1">Archive Location?</h3>
        <p class="text-sm text-slate-500 dark:text-slate-400 mb-5">
          <strong>{{ confirmArchive.name }}</strong> will be archived and removed from scheduling dropdowns.
