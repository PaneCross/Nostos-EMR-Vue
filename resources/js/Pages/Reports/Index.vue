...
onMounted(() => {
    axios
        ...
        .catch(() => {
            /* non-fatal — KPIs are supplementary */
        })
})
    ...
    return [
        {
            ...
            value: kpis.value.enrolled_participants,
            color: 'text-blue-600 dark:text-blue-400',
        },
        {
            label: 'Open Incidents',
            value: kpis.value.open_incidents,
            color: 'text-red-600 dark:text-red-400',
        },
        {
            label: 'Overdue SDRs',
            value: kpis.value.overdue_sdrs,
            color: 'text-amber-600 dark:text-amber-400',
        },
        {
            label: 'Meetings This Month',
            value: kpis.value.meetings_this_month,
        ...
        <div class="px-6 py-6 space-y-6">
            ...
            <template v-if="mainTab === 'site'">
                <div class="space-y-4">
                    <div class="flex items-center justify-between gap-4 flex-wrap">
                        <div class="flex items-center gap-3">
                            ...
                            >
                                ...
                                <option v-for="s in siteOptions" :key="s.id" :value="s.name">
                                    {{ s.name }}
                                </option>
                            </select>
                            <span v-if="siteRows" class="text-sm text-gray-500 dark:text-slate-400">
                                {{ filteredSiteRows.length }} participant{{
                                    filteredSiteRows.length !== 1 ? 's' : ''
                                }}
                    ...
                    >
                        <BuildingOfficeIcon
                            class="w-10 h-10 text-gray-300 dark:text-slate-600 mx-auto mb-3"
                            aria-hidden="true"
                        />
                        <p class="text-gray-400 dark:text-slate-500 text-sm">
                            No completed site transfers on record.
                        </p>
                    </div>

                    <div
                        v-else
                        class="overflow-x-auto rounded-xl border border-gray-200 dark:border-slate-700"
                    >
                        <table class="w-full text-sm">
                            ...
                            >
                                ...
                                >
                                    ...
                                    <td class="px-4 py-3">
                                        <span
                                            class="px-2 py-0.5 rounded text-xs font-medium bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300"
                                        >
                                            {{ row.transfer_count }}
                                        </span>
                                    </td>
                    ...
                    >
                        ...
                        <div class="flex items-start justify-between gap-2">
                            <div class="flex-1 min-w-0">
                                <span
                                    :class="[
                                        'inline-flex text-xs font-medium px-2 py-0.5 rounded mb-1.5',
                            ...
                            <DocumentChartBarIcon
                                class="w-5 h-5 text-gray-400 dark:text-slate-500 shrink-0 mt-0.5"
                                aria-hidden="true"
                            />
                        </div>
