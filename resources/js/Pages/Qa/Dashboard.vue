...
interface Kpis {
    ...
    incidents_requiring_rca: number
    overdue_rcas: number
    unsigned_notes_24h: number
    overdue_assessments: number
    open_grievances: number
    cms_reportable_grievances: number
    active_qapi_projects: number
...
const kpiCards = computed<KpiDef[]>(() => [
    {
        ...
        value: props.kpis.open_incidents,
        sublabel: 'Require attention',
        color: props.kpis.open_incidents > 0 ? 'red' : 'green',
        alert: props.kpis.open_incidents > 0,
    },
    {
        label: 'Require RCA',
        value: props.kpis.incidents_requiring_rca,
        sublabel: 'Pending root cause',
        color: props.kpis.incidents_requiring_rca > 0 ? 'amber' : 'green',
        alert: props.kpis.incidents_requiring_rca > 0,
    },
    {
        label: 'Overdue RCAs',
        value: props.kpis.overdue_rcas,
        sublabel: 'Past 30-day deadline',
        color: props.kpis.overdue_rcas > 0 ? 'red' : 'green',
        alert: props.kpis.overdue_rcas > 0,
    },
    {
        label: 'Unsigned Notes > 24h',
        value: props.kpis.unsigned_notes_24h,
        sublabel: 'Clinical documentation',
        color: props.kpis.unsigned_notes_24h > 0 ? 'amber' : 'green',
        alert: props.kpis.unsigned_notes_24h > 0,
    },
    {
        label: 'Overdue Assessments',
        value: props.kpis.overdue_assessments,
        sublabel: 'Past due date',
        color: props.kpis.overdue_assessments > 0 ? 'amber' : 'green',
        alert: props.kpis.overdue_assessments > 0,
    },
    {
        label: 'Open Grievances',
        value: props.kpis.open_grievances,
        sublabel: 'Active grievance queue',
        color: props.kpis.open_grievances > 0 ? 'amber' : 'green',
        alert: props.kpis.open_grievances > 0,
    },
    {
        label: 'CMS Reportable',
        value: props.kpis.cms_reportable_grievances,
        sublabel: 'Grievances flagged',
        color: props.kpis.cms_reportable_grievances > 0 ? 'red' : 'green',
        alert: props.kpis.cms_reportable_grievances > 0,
    },
    {
        label: 'Active QAPI Projects',
        value: props.kpis.active_qapi_projects,
        sublabel: '42 CFR §460.140',
        color: 'blue',
        alert: false,
    },
    {
        label: 'Care Plans Due Review',
        value: props.kpis.care_plans_due_review,
        sublabel: 'Within 30 days',
        color: props.kpis.care_plans_due_review > 0 ? 'amber' : 'green',
        alert: props.kpis.care_plans_due_review > 0,
    },
    {
        label: 'Open Alerts',
        value: props.kpis.open_alerts,
        sublabel: 'System alerts',
        color: props.kpis.open_alerts > 5 ? 'amber' : 'slate',
        alert: props.kpis.open_alerts > 5,
    },
    {
        label: 'Compliance Score',
        value: props.kpis.compliance_score,
        sublabel: 'Overall posture %',
        color:
            props.kpis.compliance_score >= 90
                ? 'green'
                : props.kpis.compliance_score >= 75
                  ? 'amber'
                  : 'red',
        alert: props.kpis.compliance_score < 75,
    },
])
        ...
        <div class="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">
                        42 CFR Part 460 compliance monitoring
                    </p>
                </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
                ...
                >
                    ...
                    <p class="text-xs font-medium text-gray-600 dark:text-slate-400 leading-tight">
                        {{ kpi.label }}
                    </p>
                    <p
                        :class="[
            ...
            >
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    >
                        <CheckCircleIcon
                            v-if="props.compliance_posture.baa_active"
                            class="w-4 h-4"
                        />
                        ...
                        <span class="text-xs opacity-75">
                            {{
                                props.compliance_posture.baa_active
                                    ? props.compliance_posture.baa_expires_at
                                        ? 'Expires ' + fmt(props.compliance_posture.baa_expires_at)
                                        : 'Active'
                                    : 'Missing'
                            }}
                        </span>
                    ...
                    >
                        <CheckCircleIcon
                            v-if="props.compliance_posture.sra_current"
                            class="w-4 h-4"
                        />
                        <ExclamationTriangleIcon v-else class="w-4 h-4" />
                        SRA
                        <span class="text-xs opacity-75">
                            {{
                                props.compliance_posture.sra_current
                                    ? props.compliance_posture.sra_completed_at
                                        ? 'Completed ' +
                                          fmt(props.compliance_posture.sra_completed_at)
                                        : 'Current'
                                    : 'Due'
                            }}
                        </span>
                    ...
                    >
                        <CheckCircleIcon
                            v-if="props.compliance_posture.encryption_enabled"
                            class="w-4 h-4"
                        />
                        <ExclamationTriangleIcon v-else class="w-4 h-4" />
                        Encryption at Rest
            ...
            >
                ...
                >
                    <div class="flex items-center gap-2">
                        ...
                        <span
                            class="ml-1 px-2 py-0.5 text-xs rounded-full bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300 font-medium"
                        >
                ...
                <div
                    v-if="props.openIncidents.length === 0"
                    class="px-5 py-10 text-center text-gray-500 dark:text-slate-400 text-sm"
                >
                ...
                <div v-else class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead>
                            <tr
                                class="bg-gray-50 dark:bg-slate-700/50 text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide"
                            >
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                            ...
                            >
                                <td
                                    class="px-4 py-3 font-mono text-xs text-gray-600 dark:text-slate-300"
                                >
                                    {{ incident.reference_number }}
                                </td>
                                <td class="px-4 py-3 text-gray-700 dark:text-slate-300">
                                    {{
                                        props.incidentTypes[incident.incident_type] ??
                                ...
                                <td class="px-4 py-3">
                                    <span
                                        v-if="incident.rca_required && !incident.rca_completed_at"
                                        class="flex items-center gap-1 text-amber-600 dark:text-amber-400 text-xs font-medium"
                                    >
                                    ...
                                    <span
                                        v-else-if="
                                            incident.rca_required && incident.rca_completed_at
                                        "
                ...
                <div class="p-5">
                    ...
                    <div v-if="complianceTab === 'incidents'">
                        <p class="text-sm text-gray-500 dark:text-slate-400 mb-4">
                            All open incidents are shown in the queue above. Navigate to an
                            individual incident to view details and RCA documentation.
                        </p>
                    ...
                    <div v-else-if="complianceTab === 'unsigned_notes'">
                        <div
                            v-if="unsignedNotesLoading"
                            class="py-8 text-center text-gray-500 dark:text-slate-400 text-sm"
                        >
                            Loading...
                        </div>
                        <div
                            v-else-if="unsignedNotesLoaded && unsignedNotes.length === 0"
                            class="py-8 text-center text-green-600 dark:text-green-400 text-sm flex flex-col items-center gap-2"
                        >
                        ...
                        <template v-else-if="unsignedNotesLoaded">
                            <div class="flex items-center justify-between mb-4">
                                <p class="text-sm text-gray-500 dark:text-slate-400">
                                    {{ unsignedNotes.length }} notes unsigned beyond 24 hours
                                </p>
                                <button
                                    class="text-sm text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1"
                                    @click="exportCsv('unsigned_notes')"
                                >
                            ...
                            <div class="overflow-x-auto">
                                <table class="w-full text-sm">
                                    <thead>
                                        <tr
                                            class="bg-gray-50 dark:bg-slate-700/50 text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase"
                                        >
                    ...
                    <div v-else-if="complianceTab === 'overdue_assessments'">
                        <div
                            v-if="overdueAssessmentsLoading"
                            class="py-8 text-center text-gray-500 dark:text-slate-400 text-sm"
                        >
                            Loading...
                        </div>
                        <div
                            v-else-if="overdueAssessmentsLoaded && overdueAssessments.length === 0"
                            class="py-8 text-center text-green-600 dark:text-green-400 text-sm flex flex-col items-center gap-2"
                        >
                            <CheckCircleIcon class="w-8 h-8" />
                        ...
                        <template v-else-if="overdueAssessmentsLoaded">
                            <div class="flex items-center justify-between mb-4">
                                <p class="text-sm text-gray-500 dark:text-slate-400">
                                    {{ overdueAssessments.length }} overdue assessments
                                </p>
                                <button
                                    class="text-sm text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1"
                                    @click="exportCsv('overdue_assessments')"
                                >
                                    <ArrowDownTrayIcon class="w-4 h-4" /> Export
                                </button>
                            </div>
                    ...
                    <div v-else-if="complianceTab === 'grievances'">
                        <div
                            v-if="grievancesLoading"
                            class="py-8 text-center text-gray-500 dark:text-slate-400 text-sm"
                        >
                            Loading...
                        </div>
                        <div
                            v-else-if="grievancesLoaded && grievances.length === 0"
                            class="py-8 text-center text-green-600 dark:text-green-400 text-sm flex flex-col items-center gap-2"
                        >
                            <CheckCircleIcon class="w-8 h-8" />
                            No open grievances.
                        ...
                        <template v-else-if="grievancesLoaded">
                            <div class="flex items-center justify-between mb-4">
                                <p class="text-sm text-gray-500 dark:text-slate-400">
                                    {{ grievances.length }} open grievances
                                </p>
                                <button
                                    class="text-sm text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1"
                                    @click="exportCsv('grievances')"
                                ...
                                <table class="w-full text-sm">
                                    ...
                                    <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                                        <tr
                                            ...
                                            :key="g.id"
                                            class="hover:bg-gray-50 dark:hover:bg-slate-700/40 cursor-pointer"
                                            @click="router.visit(`/grievances/${g.id}`)"
                                        >
                                            <td
                                                class="px-4 py-3 font-mono text-xs text-gray-600 dark:text-slate-300"
                                            >
